...
(
    SELECT
        ...
        FROM (
            SELECT
                shipment_id,
                MIN(CASE WHEN status IN (13,39,42,8) THEN DATE(FROM_UNIXTIME(ctime-3600)) 
...
(
    select 
        ...
        ,CASE 
            WHEN station.station_type = 5 OR fleet_order.channel_id != 1 OR station.station_name LIKE '4PL%' OR timestamp_4pl is not null THEN 1 
            ELSE 0 
        END AS is_4pl
...
select 
    temp_date.report_date
    ,total_pickup_volume
    ,total_spx_serviceable_volume
    ,cast(spx_e2e_servicable_volume as double)/total_spx_serviceable_volume as per_aloocated_spx 
    ,cast(servicable_volume_4pl as double)/total_spx_serviceable_volume as per_aloocated_4pl 
    ,total_spx_non_serviceable_volume
from temp_date
left join agg_servicable
on temp_date.report_date = agg_servicable.report_date
order by 
    temp_date.report_date desc 
