...
left join 
    ...
    from 
        ...
        where 
            status = 42 and date(from_unixtime(ctime - 3600)) >= current_date - interval '40' day 
        )
    where 
        rank_num = 1 
    ) as fm_path 
...
LEFT JOIN 
    ...
    from 
        ...
        FROM spx_mart.shopee_ssc_spx_track_th_db__order_tracking_tab__reg_continuous_s0_live
        WHERE 
            status IN (4,81) AND operator NOT LIKE '%SPX%' AND DATE(FROM_UNIXTIME(ctime-3600)) >= DATE('2021-09-01')
    )
    where rank_num = 1 
...
LEFT JOIN 
    ...
    from 
        ...
        FROM spx_mart.shopee_ssc_spx_track_th_db__order_tracking_tab__reg_continuous_s0_live
        WHERE 
            status IN (58,95) AND DATE(FROM_UNIXTIME(ctime-3600)) >= DATE('2021-09-01')
        )
    where 
        rank_num = 1 
...
select 
    case 
        when returned_date between date('2022-05-01') and date('2022-05-31')  then 'May22'
        when returned_date between date('2022-04-01') and date('2022-04-30')  then 'Apr22'
        when returned_date between date('2022-03-01') and date('2022-03-31')  then 'Mar22'
    end as report_date
    -------- volume 
    ...
    ---------- cost 
    ,0.5*SUM(CASE WHEN delivery_station = 'Flash Express' then cast(rate.rate_card as int) else 0 end) as flash_rev_fee
    ,0.5*SUM(CASE WHEN delivery_station = 'Flash Express' and tier = 'Tier A' then cast(rate.rate_card as int) else 0 end) as flash_ta_rev_fee
    ,0.5*SUM(CASE WHEN delivery_station = 'Flash Express' and tier = 'Tier B' then cast(rate.rate_card as int) else 0 end) as flash_tb_rev_fee
    ,0.5*SUM(CASE WHEN delivery_station = 'Flash Express' and tier = 'Tier C' then cast(rate.rate_card as int) else 0 end) as flash_tc_rev_fee
    ,0.5*SUM(CASE WHEN delivery_station = 'Flash Express' and tier = 'Tier D' then cast(rate.rate_card as int) else 0 end) as flash_td_rev_fee
    ,0.5*SUM(CASE WHEN delivery_station = 'Flash Express' and tier = 'Tier E' then cast(rate.rate_card as int) else 0 end) as flash_te_rev_fee
    ,0.5*SUM(CASE WHEN delivery_station = 'Flash Express' and tier = 'Tier F' then cast(rate.rate_card as int) else 0 end) as flash_tf_rev_fee
...
where 
    returned_date between date('2022-05-16') and date('2022-05-22') 
    --current_date - interval '35' day and current_date - interval '1' day 
group by 1 
)
...
SELECT
    case 
        when delivered_date between date('2022-05-01') and date('2022-05-31')  then 'May22'
        when delivered_date between date('2022-04-01') and date('2022-04-30')  then 'Apr22'
        when delivered_date between date('2022-03-01') and date('2022-03-31')  then 'Mar22'
    end as report_date
    ...
    ---------------------------- Cost 
    ,SUM(CASE WHEN delivery_station = 'Flash Express' then cast(rate.rate_card as int) else 0 end) as flash_cost
    ,SUM(CASE WHEN delivery_station = 'Flash Express' and tier = 'Tier A' then cast(rate.rate_card as int) else 0 end) as flash_ta_cost
    ,SUM(CASE WHEN delivery_station = 'Flash Express' and tier = 'Tier B' then cast(rate.rate_card as int) else 0 end) as flash_tb_cost
    ,SUM(CASE WHEN delivery_station = 'Flash Express' and tier = 'Tier C' then cast(rate.rate_card as int) else 0 end) as flash_tc_cost
    ,SUM(CASE WHEN delivery_station = 'Flash Express' and tier = 'Tier D' then cast(rate.rate_card as int) else 0 end) as flash_td_cost
    ,SUM(CASE WHEN delivery_station = 'Flash Express' and tier = 'Tier E' then cast(rate.rate_card as int) else 0 end) as flash_te_cost
    ,SUM(CASE WHEN delivery_station = 'Flash Express' and tier = 'Tier F' then cast(rate.rate_card as int) else 0 end) as flash_tf_cost
...
AND order_map.delivery_station = rate.shipment_provider
where 
    delivered_date between date('2022-05-16') and date('2022-05-22')  
--current_date - interval '35' day and current_date - interval '1' day 
group by 1 
)
select 
